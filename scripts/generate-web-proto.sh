...
UI_ROOT="${UI_ROOT:-${ROOT}/ui}"
PROTO_DIR="${PROTO_DIR:-${ROOT}/proto}"
OUT_DIR="${OUT_DIR:-${UI_ROOT}/src/proto}"
LOCAL_PLUGIN="${UI_ROOT}/node_modules/.bin/protoc-gen-ts_proto"
...
if [[ -z "${PLUGIN}" || ! -x "${PLUGIN}" ]]; then
  printf 'ts-proto plugin not found; run npm install in common-lib/ui or webui first\n' >&2
  exit 1
fi
