...
export function useMailboxEmailEventCache(options: MailboxEmailEventCacheOptions) {
  ...
  const enabled = options.enabled !== false && !!email && !!options.inboxQueryKey;
  useHotStreamInvalidation({
    enabled,
    url: mailboxEventURL(options.apiBase, email, options.signalKind, options.issuedAfterUnix),
    rules: options.inboxQueryKey ? [{
      queryKey: options.inboxQueryKey,
      eventTypes: options.signalKind === 'any' ? ['mailbox.email.received', 'mailbox.email.signal_received'] : ['mailbox.email.signal_received'],
      resourceTypes: ['mailbox.email'],
      resourceIds: [email]
    }] : []
  });
}
...
export function mailboxEventURL(apiBase = '/api/mailbox', email: string, signalKind = 'otp', _issuedAfterUnix = 0) {
  const eventTypes = signalKind === 'any' ? ['mailbox.email.received', 'mailbox.email.signal_received'] : ['mailbox.email.signal_received'];
  return createHotStreamURL(apiBase, { eventTypes, resourceTypes: ['mailbox.email'], resourceIds: [email] });
}
